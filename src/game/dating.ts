...
const BIOS = [
  ...
  'Swipe right if you can beat me at kart racing.',
...
export const VIP_JOBS: { job: string; emoji: string; salary: [number, number] }[] = [
  ...
  { job: 'Pro Basketball Star', emoji: '🏀', salary: [20, 50] },
  ...
  { job: 'Racing Driver', emoji: '🏎️', salary: [10, 60] },
  { job: 'Award-winning Director', emoji: '🏆', salary: [5, 30] },
...
export function makeProfile(g: Game, vip: boolean): Profile {
  ...
  return {
    ...
    education: vip ? pick(['Ivy League dropout', 'Top-university graduate', 'Performing arts school alum', 'Self-made', 'Private tutors']) : age < 18 ? 'High school' : eduFor(salary),
...
export function makeRoyalProfile(g: Game): Profile {
  ...
  return {
    ...
    education: pick(['Royal Academy', 'Private tutors at the palace', 'A top university, then the Royal Academy', 'Military academy']),
    bio: pick(ROYAL_BIOS),
